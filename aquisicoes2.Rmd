...
aquisicoes_completo <- read_excel("ajustados_2_linha/piloto0806.xlsx", col_types = c("text", "text", "text", 
                                                                                    "text", "text", "text",
                                                                                    "text", "text", "text", 
                                                                                    "text", "text", "text", 
                                                                                    "text","text","numeric", 
...
aquisicoes_completo$intervalo <- aquisicoes_completo$date %--% dmy("08-06-2020") 
aquisicoes_completo$intervalo <- aquisicoes_completo$intervalo / ddays(1)
teste_data <- aquisicoes_completo %>% filter(intervalo<180)
...
respiradores <- respiradores %>% mutate (qtde_percapita = quantidade/pop)
barras_respiradores <- respiradores %>% 
  ...
  theme(
    ...
    legend.position="none") +
  xlab("") +
  ylab("Respiradores adquiridos para cada habitante")
...
[AQUI](https://github.com/rodrigoeloy/CGU/raw/master/piloto0806.xlsx).   

As análises foram realizadas na linguagem R e essa página foi elaborada com o RMarkdown. 

Atualmente, são considerados nessas análises `r nrow(aquisicoes_completo)` registros de contratações relacionadas ao enfrentamento da COVID-19, alcançando o montante de R$ `r sum(aquisicoes_completo$Valor_Total)/1000000000` bilhões.  

Desse total, R$ `r sum(aquisicoes_compa$Valor_Total)/1000000000` bilhões (`r ((sum(aquisicoes_compa$Valor_Total)/1000000000) / (sum(aquisicoes_completo$Valor_Total)/1000000000))*100` %) são referentes aos aqui denominados "itens padronizáveis", que são aqueles objetos contratados em larga escala para o enfrentamento da COVID e que podem ser comparados entre si, tais como álcool em gel, máscaras e ventiladores mecânicos (respiradores).  
...
No início de cada parte consta uma introdução sobre os objetivos das análises contidas naquela seção, bem como os principais conceitos, metodologias e limitaçoes conhecidas. Assim como ocorre com o volume de dados coletados, as análises aqui contidas são dinâmicas e atualizadas regularmente.
...
Com base nesses conceitos, as variações dos preços unitários estão representadas por item (considerando o total de registros daqueles itens em todo o país) e por UF. Tambem consta nessa seção um mapa do Brasil sobre os scores padrão médio calculado para cada UF.   
...
O conjuto de gráficos dessa seção representam as variações dos preços unitários dos itens comparáveis. Nesse caso, os preços são apresentados por UF, possibilitando a análise regionalizada para cada item.
...
A primeira aba consiste em graficos de barras da quantidade de itens adquiridos em cada UF ponderada pela quantidade de habitantes (conforme estimativa do IBGE para 2019).  
...
## INTRODUÇÃO
Nessa parte constam análises relacionadas às entidades públicas responsáveis pelas contratações tabuladas. Ao agrupar os registros pelas entidades contratantes, é possível identificar quais órgãos têm operacionalizados o maior volume de contratações para enfrentamento da Covid-19 (em termos financeiros). Para esse tipo de análise, foram considerados todos os registros (itens padronizáveis e não padronizáveis).  

Além disso, foram calculados os escores padrões médios dos contratantes, o que aponta para os órgãos que, considerando os registros de contratações dos itens padronizáveis, têm praticados os preços unitários mais elevados em relação à média praticada no país. De igual modo, esse tipo de análise indica os órgãos que têm praticado preços inferiores à média.  
...
## INTRODUÇÃO
De modo semelhante às análises em relação aos orgãos contratantes, essa seção sintetiza informações sobre os principais fornecedores (em termos de valores), bem como indica aqueles que apresentam escores padrões médio acima ou abaixo da média.  

Além das análises sobre o montante total contratado e as variações dos preços unitários, foram identificadas situações de potencial criticidade envolvendo as empresas contratadas. Tais situações dizem respeito à contratação de empresas recém criadas e a contratação de grande vulto com empresas de pequeno porte. Os critérios para a identificação de riscos das contratações indicados nessa seção estao em constante construção e aprimoramento.  
...
Nessa seção constam fornecedores que apresentam níveis elevados de criticidade, de acordo com os seguintes criterios:  
  
- Pessoas Jurídicas criadas após junho de 2019  
...
```{r results = 'asis',echo=FALSE} 
print(xtable::xtable(teste_data[,c(9,13,14,15,29)]),type = "html", html.table.attributes = "border=1")
``` 
...
- Dessa forma, a proporção entre o montante efetivamente gasto e o montante estimado tendo como limite as medianas do preço unitário é de `r sum(aquisicoes_compa$valor_limite)/sum(aquisicoes_compa$valor_total_padronizado)` %

- Ao considerar também as contratações não padronizáveis, o montante total chega a R$ `r sum(aquisicoes_completo$Valor_Total)/1000000000` bilhões

- Utilizando a mesma proporção calculada anteriormente, tem-se que o montante total poderia ser de R$ `r (taxa*sum(aquisicoes_completo$Valor_Total))/1000000000` bilhões

**Conclusão: Estima-se que os sobrepreços (valores acima da mediana) ocorridos nas aquisições de enfrentamento da COVID-19 implicam na majoração do montante de despesas executadas na ordem de R$ `r ((sum(aquisicoes_completo$Valor_Total))-(taxa*sum(aquisicoes_completo$Valor_Total)))/1000000000` bilhões**
